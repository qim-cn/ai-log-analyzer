...
1. **给排查命令**：先给用户可以在机器上直接执行的命令，定位到底是部件问题还是槽位/背板/主板问题。例如 PCIe 降宽度时给 `lspci -vvv` + `setpci` 等诊断命令，而不是猜测
2. **重插拔 + 目检**：建议重新插拔故障部件，检查金手指/针脚/连接器
3. **交叉验证**：将该部件换到另一个已知正常的槽位/机器上测试
4. **换件**：仅当前三步都无法解决时，才更换故障部件。换件后重跑测试是标准流程，无需重复
5. **批量风险**：同一工位/同一机型连续出现相同故障模式 → 升级为质量事件
