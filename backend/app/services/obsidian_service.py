...
"""
    ...
    async def _list_directory(self, base_url, auth, dir_url, current_path, vault_path):
        ...
        for item in items:
            ...
            # 构建相对路径
            rel_path = href.replace(f"/{vault_path}/", "").strip("/")
    ...
    async def _get_file_content_webdav(self, path: str) -> Optional[str]:
        ...
        clean_path = path
        if clean_path.startswith("Obsidian Vault/"):
            clean_path = clean_path[len("Obsidian Vault/"):]

        if clean_path.startswith(vault_path):
            file_url = _make_webdav_url(base_url, clean_path)
        else:
            file_url = _make_webdav_url(base_url, f"{vault_path}/{path}")
